        ...
        while let Ok((path, result)) = self.rx.try_recv() {
            ...
            match result {
                Ok((image, size)) => {
                    ...
                    self.textures
                        .insert(path, ThumbnailTexture { handle, size });
                }
                Err(_) => {
...
fn scan_directory_fast(dir: &Path) -> Vec<AssetEntry> {
    ...
    let mut entries: Vec<AssetEntry> = read_dir
        ...
        .filter_map(|entry| {
            ...
            // Filter for USD and glTF file types
            let ext = path.extension()?.to_str()?.to_lowercase();
            if matches!(
                ext.as_str(),
                "usd" | "usda" | "usdc" | "usdz" | "gltf" | "glb"
            ) {
    ...
    pub fn show(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) -> Option<PathBuf> {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.separator();
                if ui
                    .checkbox(&mut self.hide_texture_folders, "Hide Texture Folders")
                    .changed()
                {
                    self.apply_filter();
                }
            ...
            .show(ui, |ui| {
                ...
                if self.current_dir.is_none() {
                            ...
                            AssetViewMode::Grid => {
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    for (idx, path) in paths.iter() {
                                        ...
                                        let (thumb_grid, thumb_cover) =
                                            find_thumbnail_paths(path, true);
                            ...
                            AssetViewMode::Cover => {
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    for (idx, path) in paths.iter() {
                                        ...
                                        let card_id =
                                            egui::Id::new(("lib_root_cover", path.as_path()));
                    ...
                    if self.filtered.is_empty() {
                        ui.label("No assets found (USD/glTF).");
                    } else {
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    for entry in entries.iter() {
                                        ...
                                        let thumb = entry
                                            .thumbnail_grid
                                            .as_ref()
                                            .or(entry.thumbnail_cover.as_ref());
                                ...
                                ui.horizontal_wrapped(|ui| {
                                    for entry in entries.iter() {
                                        ...
                                        let thumb = entry
                                            .thumbnail_cover
                                            .as_ref()
                                            .or(entry.thumbnail_grid.as_ref());
